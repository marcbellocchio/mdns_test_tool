                ...
                (temp.startsWith("-") || temp.startsWith("--")))
                {
                    ...
                    switch (option)
                    {
                        ...
                        case 'F':
                            ...
                            System.out.println("Mydnssd Browse Domains:");
                            printArray(domains, "\t%s\n");
                            System.out.println("\n" + timingBuilder.toString() + " - took " + ExecutionTimer._took(TimeUnit.SECONDS) + " seconds.");
                            break;
                        case 'B':
                        ...
                        case 'W':
                        	...
                        	Record[] records;                       	
                        	if (args.length > 2)
                        		throw new IllegalArgumentException("Invalid option 2 parameters only -W <timeout> ");
                        		
                        	
                        	type = Type.value("PTR", true);
                            ...
                            {
                            	if (args.length == 1)
                            		records = lookup.lookupRecords(WAITING_TIME_RESPONSE,2);
                            	else
                            		records = lookup.lookupRecords(Integer.parseInt(args[1]),2);
